...
const checkLogin = (req, res, next) => {
    ...
    if (!req.session.user._id || !req.session.user.userName || !req.session.user.userImage) {
        return res.status(401).json({ 'resultType': 'ERROR', 'resultMsg': 'User is required.' });
    }
    next();
};
...
const checkCreateBlogArgs = (title, content) => {
...
const checkPushCommentArgs = (blogId, message) => {
